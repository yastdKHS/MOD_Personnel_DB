...
> Phase2 Task1（Repository Skeleton）・Task2（Domain Model Implementation）・Task3（Pipeline Skeleton Implementation）・Task4（Document Analyzer Implementation）・Task5（Layout Detector Implementation）より実装着手。`utils/`, `models/`（[`docs/api/models.md`](../docs/api/models.md)の13モデル全種＋Layout Detector関連の追加型。`Document`はVersion 2.0（[ADR-0032](../docs/adr/0032-redefine-document-analyzer-responsibility.md)）で`file_path`を保持し（[ADR-0035](../docs/adr/0035-layout-detector-owns-pdf-content-access.md)）、Version 1は`DocumentV1`/`PageV1`として引き続き提供）, `repositories/`（Protocol）, `repositories/sqlite/`（SQLite実装、7種）, `pipeline/`（Pipeline Framework骨格）, `document/`（Document Analyzer本実装。DocumentAnalyzer/DocumentAnalyzerError、PDF解析ライブラリは`pypdf`（[ADR-0034](../docs/adr/0034-pypdf-for-document-analyzer.md)）), `layout/`（Layout Detector本実装。LayoutDetector/LayoutDetectorError、`document.file_path`を用いたPDF再読込・様式判定を唯一担う（[ADR-0035](../docs/adr/0035-layout-detector-owns-pdf-content-access.md)）。`LayoutDefinition`のYAMLロードは`pyyaml`（[ADR-0036](../docs/adr/0036-pyyaml-for-layout-definition.md)）。中核パイプライン段階3以降＝`sections/`〜`validators/`は未実装のまま）が実装済み。`config/`、`sections/`〜`validators/`（中核パイプライン段階3〜6）, `learning/`, `features/`, `review/`（Domain Service）, `export/`, `ftp/`, `fetch/`, `services/`, `cli/` は未実装。
