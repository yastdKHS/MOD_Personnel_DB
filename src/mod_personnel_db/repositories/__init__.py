...
class CandidateRepository(Protocol):
    ...
    # Task31 Step6/Step7: 再実行安全性（Idempotency）向けの検索・Supersede API。
    # parser_version_idは公開引数に含めない（実装は束縛済みの値を内部で使う、
    # Task31 Step4のハイブリッド方式）。
    def find_section(self, pdf_id: PdfId, section_index: int) -> PersonnelSectionId | None: ...
    def find_active_section(
        self, pdf_id: PdfId, section_index: int
    ) -> PersonnelSectionId | None: ...
