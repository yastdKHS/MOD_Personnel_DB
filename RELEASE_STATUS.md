...
2026-07-22（Phase6 Task15-4作成時点）。2026-07-24、Task17-5（Phase7最終監査・CLI統合完了後のドキュメント同期）・Task17-6（Phase7 Closeout）で更新。2026-07-25、Task18-1（`FtpSettings`導入）・Task18-3〜18-5（GitHub Actions Scheduler統合）・Task18-6（Production FTP運用整備）・Task18-7（Production Release Validation、[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）・Task18-7追記（運用担当者による実Production FTPサーバへの手動接続確認、`remote_directory`未配線の発見。続報でフルパス指定によるアップロード成功も確認）で更新。
...
15. **2026-07-25追記**: 運用担当者が実際のProduction FTPサーバに対して`run-workflow --remote-path`を手動実行したところ、`connect()`/`login()`は成功したが、`upload()`（`STOR`）が絶対パス指定時に`530 You cannot upload file here`で失敗した。コード読解の結果、`FtpSettings.remote_directory`（`MOD_PERSONNEL_DB_FTP__REMOTE_DIRECTORY`に対応）は`FTPConnectionConfig`（`ftp/config.py`）にフィールドが存在せず、`build_ftp_client()`（`cli/bootstrap.py`）でもマッピングされず、`StandardFTPClient.connect()`（`ftp/client.py`）も`cwd()`を呼ばないため、**実行時に一切参照されない**ことが判明した。**続報**: FTPアカウントの許可ディレクトリを含むフルパスを`--remote-path`に指定して再実行したところ、**アップロードは成功した**。したがって`remote_directory`未配線はアップロードの機能的なブロッカーではなく、「`_REMOTE_DIRECTORY`という設定項目自体が実行時に一切効果を持たないデッドコンフィグである」という設定整合性上の課題として確定した（詳細は[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)の「5. FTP Upload確認」「8. Known Limitations確認」参照）。是正（`remote_directory`の`cwd()`反映、または項目自体の削除・仕様明確化）には`src/**`の変更を要するため、本Taskでは実施していない。
...
- `ftp/`の実接続情報整備は`config/`への`FtpSettings`追加・`build_ftp_client()`更新（Task18-1）、対応するGitHub Secrets整備（Task18-6）まで完了した。2026-07-25、運用担当者による実Production FTPサーバへの手動接続確認で、**接続・認証（`connect()`/`login()`）およびアップロード（`STOR`、許可ディレクトリを含むフルパスを`--remote-path`に指定）の成功がいずれも実測で確認された**。これによりProduction FTPサーバへの手動公開経路（`run-workflow --remote-path`）は実際に機能することが実証された。副次的に、`FtpSettings.remote_directory`がコード上（`FTPConnectionConfig`・`build_ftp_client()`・`StandardFTPClient.connect()`のいずれにも）配線されておらず実行時に一切参照されないという事実も判明したが、`--remote-path`にフルパスを指定する運用で回避可能であり、アップロード自体のブロッカーではないことも確認済みである（[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）。残るのは、（a）`remote_directory`という設定項目自体の整合性是正（`cwd()`反映または項目削除・仕様明確化、`src/**`変更を要するため新規Task）、（b）GitHub Actions（`scheduler.yml`/`release.yml`）経由での自動実行実績の確認、の2点である
...
- **Task18-7 Production Ready判定: Release Candidate maintained（2026-07-25の2度の追記後も変更なし、根拠を更新）。** 当初のGitHub Actions API・実CLI実行による実測検証（[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）では、（1）実FTPサーバへの接続実績、（2）GitHub Secretsの実登録状況、（3）`scheduler.yml`のGitHub Actions runner上での実行実績、の3点が「未確認」のまま残っていた。その後、運用担当者が実際にProduction FTPサーバへの手動接続確認を2度実施した結果、**（1）は接続・認証・アップロードのいずれも成功として確認済みに更新された**（1回目は絶対パス指定で`530`エラー、2回目は許可ディレクトリを含むフルパス指定で成功）。（2）（3）は引き続き未確認のままである。FTP公開経路（手動実行）が実サーバに対して機能することが実証されたのは重要な進捗だが、（i）GitHub Actionsを経由した自動実行での動作は依然未検証、（ii）FTP公開自体が`schedule-now`の自動実行経路には含まれない設計上の制約は変わらない、（iii）`remote_directory`という設定項目が実行時に一切効果を持たないデッドコンフィグである点、（iv）データ整備・セキュリティ強化等の他のKnown Limitationsも未解消、の4点により、Release Candidate maintainedの判定を継続する。
