...
22. **（Task22-1〜22-3、Task22-8で解消済み）** `version_command`のConnection管理が他コマンドと異なる問題: `cli/commands.py::version_command()`が`build_application(settings)`を`connection`引数なしで呼び出すため内部で生成したConnectionが`try/finally`でcloseされない、あわせて`version`表示に不要なRepository（`jobs`以外の6種・Candidate Repository）・JobRunnerが毎回生成される、という2点が残っていた。Task22-1でclose責務を他コマンドと統一し、Task22-2/22-3で`version`専用の軽量Builder（`build_version_dependencies()`、`FileKnowledgeService`と`SqliteJobRepository`のみを組み立てる）を導入して不要な生成を解消した。Task22-8では`version_dependencies_session()`（Context Manager方式のSession Builder）へさらに整理し、Connection生成・close責務をComposition Root（`cli/bootstrap.py`）へ完全集約した。旧「Technical Debt（Future Refactoring Candidates）」節の項目1に対応する改善はこれで完了している（下記Technical Debt節参照）。
...
SQLite Connection管理（`cli/bootstrap.py`・`cli/commands.py`）に関するTask21レビューで整理した項目。**項目1〜5はTask22-1〜22-8ですべて解消済みである。** 項目6は不採用のまま参考記録として残す。

1. **（Resolved, Task22-2/22-3）** `version_command`専用の軽量Builder導入: `bootstrap.py`に`version_command`専用のBuilder`build_version_dependencies()`（`FileKnowledgeService`と`SqliteJobRepository`のみを組み立てる関数）を追加し、`build_application()`が持つ不要なRepository生成・JobRunner構築・`parser_versions`書き込み副作用を回避した。Composition Root一本化原則（Architecture Contract Guarantee 15）を維持したまま実装済み（Task22-8で`version_dependencies_session()`としてさらにSession Builder化）。
2. **（Resolved, Task22-5/22-6）** `_build_job_orchestrator()`のRepository二重生成解消: `build_application()`が生成する`SqlitePdfRepository`（Connection共有済み）を`_build_job_orchestrator()`側でも再利用できるよう`build_application_with_repositories()`を追加し、`build_sqlite_repositories()`の2回目呼び出し（jobs/gold/knowledge/review/export/learning 6 Repository分の無駄な生成）を排除した。
3. **（Resolved, Task22-8）** Connectionを`commands.py`へ露出しない設計への整理: `application_session()`/`job_orchestrator_session()`/`scheduler_session()`/`version_dependencies_session()`（`cli/bootstrap.py`のContext Manager方式Session Builder）を導入し、`cli/commands.py`が`sqlite3.Connection`型・`repositories.sqlite.connect()`を扱わない設計へ整理した（`_check_database_integrity()`のみ意図的な例外として維持）。
4. **（Resolved, Task22-8）** `tuple[JobOrchestrator, sqlite3.Connection]`の廃止: 上記3の帰結として、`_build_job_orchestrator()`/`_build_scheduler()`（および両者が返していたtuple形式の戻り値）を廃止し、Session Builderが`JobOrchestrator`/`Scheduler`単体をyieldする形へ整理した。
5. **（Resolved, Task22-8）** Connection生成・所有・close責務をComposition Rootへ完全集約: 生成者（`commands.py`内の旧`_build_job_orchestrator()`）とclose責務者（呼び出し元コマンド関数）が分離していた状態を解消し、`bootstrap.py`のSession Builder（Context Manager、`connect()`→`build_xxx()`→`yield`→`finally: connection.close()`）が生成・close責務の両方を完結して持つ設計へ統一した。
