...
| Phase7 | `features/`・`fetch/`・`ftp/`・`services/`の設計方針確定・依存方向明文化・実装ロードマップ作成（Task16-0、[`docs/phase7-implementation-roadmap.md`](docs/phase7-implementation-roadmap.md)）、および4パッケージの実装（Task16-1〜16-4）・最終監査（Task16-5、読み取り専用）・ドキュメント同期（Task16-6）。Composition Root（`cli/`）への配線は本Phaseの対象外。 | 完了 |
...
3. `features/`・`ftp/`・`fetch/`・`services/`パッケージはPhase7（Task16-1〜16-4）で実装済みである（[`docs/api/package-design.md`](docs/api/package-design.md)参照）。しかし、これら4パッケージはComposition Root（`cli/bootstrap.py`）へ未配線であり、CLIコマンドとして利用できない。CLI統合（`cli/`からの呼び出し配線）・Scheduler（定期実行トリガー管理、`docs/api/interfaces.md`の`Scheduler`Protocol）の実装がいずれも未着手であるため、PDFの自動取得・実際のデータベース公開（FTP送信等）に至る自動化された経路は依然として存在しない。`features/`はさらに`pipeline/`（`JobRunner`）からも呼び出されておらず、独立した未接続のパッケージのままである。
...
- Phase7で実装済みの`features/`・`ftp/`・`fetch/`・`services/`パッケージのComposition Root（`cli/`）への配線・CLI統合の**実装**、および`Scheduler`（定期実行トリガー管理）の実装。統合の**設計**自体はTask17-0（[`docs/phase7-integration-design.md`](docs/phase7-integration-design.md)）で完了しており、`FTPClient`統合は`config/`への`FtpSettings`追加（未実装）が前提条件として残る
...
- **v1.0.0タグ付与（正式リリース）は推奨しない。** 上記Known Limitations、特に(a)`features/`・`ftp/`・`fetch/`・`services/`はPhase7で実装済みだがComposition Root未配線・CLI未統合・Scheduler未実装によりPDF自動取得・実際の外部公開経路が存在しないこと、(b)`layouts/`・`knowledge/`・Golden Testフィクスチャが実運用規模に未到達であること、(c)ADR-0026・ADR-0029が求めるセキュリティ関連実装が未着手であること、の3点は「継続的にPDFを収集・公開する」というプロジェクトの中核目的に直接関わるため、これらの解消を待つべきである。
...
- [`docs/phase7-implementation-roadmap.md`](docs/phase7-implementation-roadmap.md) — Phase7未実装4パッケージの設計方針・実装ロードマップ（Task16-0）
