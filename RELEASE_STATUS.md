...
2026-07-22（Phase6 Task15-4作成時点）。2026-07-24、Task17-5（Phase7最終監査・CLI統合完了後のドキュメント同期）・Task17-6（Phase7 Closeout）で更新。2026-07-25、Task18-1（`FtpSettings`導入）・Task18-3〜18-5（GitHub Actions Scheduler統合）・Task18-6（Production FTP運用整備）・Task18-7（Production Release Validation、[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）・Task18-7追記（運用担当者による実Production FTPサーバへの手動接続確認、`remote_directory`未配線の発見）で更新。
...
15. **2026-07-25追記**: 運用担当者が実際のProduction FTPサーバに対して`run-workflow --remote-path`を手動実行したところ、`connect()`/`login()`は成功したが、`upload()`（`STOR`）が`530 You cannot upload file here`で失敗した。コード読解の結果、`FtpSettings.remote_directory`（`MOD_PERSONNEL_DB_FTP__REMOTE_DIRECTORY`に対応）は`FTPConnectionConfig`（`ftp/config.py`）にフィールドが存在せず、`build_ftp_client()`（`cli/bootstrap.py`）でもマッピングされず、`StandardFTPClient.connect()`（`ftp/client.py`）も`cwd()`を呼ばないため、**実行時に一切参照されない**ことが判明した（詳細は[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)の「5. FTP Upload確認」「8. Known Limitations確認」参照）。是正には`src/**`の変更を要するため、本Taskでは実施していない。
...
- `ftp/`の実接続情報整備は`config/`への`FtpSettings`追加・`build_ftp_client()`更新（Task18-1）、対応するGitHub Secrets整備（Task18-6）まで完了した。2026-07-25、運用担当者による実Production FTPサーバへの手動接続確認で、**接続・認証（`connect()`/`login()`）の成功が実測で確認された**。一方、**アップロード（`STOR`）は`530`エラーで失敗**し、`FtpSettings.remote_directory`がコード上（`FTPConnectionConfig`・`build_ftp_client()`・`StandardFTPClient.connect()`のいずれにも）配線されておらず実行時に一切参照されないという新規欠陥が判明した（[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）。残るのは、この欠陥の是正（`remote_directory`の`cwd()`反映、または`--remote-path`との統合方針の設計。`src/**`変更を要するため新規Task）と、FTPサーバ側の書き込み許可ディレクトリの確認である
...
- **Task18-7 Production Ready判定: Release Candidate maintained（2026-07-25追記後も変更なし、根拠を更新）。** 当初のGitHub Actions API・実CLI実行による実測検証（[`docs/reports/phase8-release-validation.md`](docs/reports/phase8-release-validation.md)）では、構造面・実装面の欠陥は確認されなかった一方、（1）実FTPサーバへの接続実績、（2）GitHub Secretsの実登録状況、（3）`scheduler.yml`のGitHub Actions runner上での実行実績、の3点が「未確認」のまま残っていた。その後、運用担当者が実際にProduction FTPサーバへの手動接続確認を実施した結果、（1）は**接続・認証の成功として確認済み**に更新されたが、アップロード自体は`remote_directory`未配線という**新規のコード欠陥**により`530`エラーで失敗した。（2）（3）は引き続き未確認のままである。「未確認」から「実測に基づく具体的な欠陥」へと根拠が具体化したため、Release Candidate maintainedの判定は継続するが、v1.0.0 Readyへの昇格には、GitHub Secrets実登録・cron実行実績の確認に加えて**`remote_directory`未配線の是正（新規Task、`src/**`変更を伴う）**が新たな前提条件として加わった。
