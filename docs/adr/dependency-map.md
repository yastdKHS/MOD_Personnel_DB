...
> `A --> B` は「AはBを参照・前提としている（Aを理解するにはBの決定を先に理解する必要がある）」ことを表す。矢印の向きは各ADR本文中の「関連ADR」節・コンテキスト中の参照から機械的に抽出したもの（抽出方法は[品質チェックレポート](quality-check.md#依存関係の抽出方法)を参照）。新規ADR（0018〜0026）の依存関係は、[`gap-analysis.md`](gap-analysis.md)での検討過程に基づき設計時点で付与したもの。ADR-0027（Review Domainの中核化）・ADR-0028（設定管理へのPydantic Settings採用）の依存関係は、当該ADR本文の「コンテキスト」節に基づき付与した。
...
以下は本ファイルの全65エッジ（ADR-0028追加後）を実際に集計した値である（[品質チェックレポート](quality-check.md#依存関係の抽出方法)で検証済み。0027追加時点からの増分はADR-0028の新規3エッジのみで、上位ランキングの顔ぶれには変動なし）。
...
被参照数0のADR（`0018`, `0020`, `0022`, `0023`, `0024`, `0027`, `0028`、および既存の`0009`）については [`quality-check.md`](quality-check.md#孤立したadr) で個別に評価している。いずれも参照数（out-degree）は1以上あり、グラフ上完全に孤立したノード（in=0 かつ out=0）は存在しない（28ノード・65エッジで再検証済み）。`0021`は`0027`（Review Domainの中核化）から、`0026`は`0028`（Pydantic Settings採用）から新たに参照されたため、この一覧から外れた。
