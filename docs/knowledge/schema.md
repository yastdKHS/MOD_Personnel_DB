...
| `layout` | Section Parser / Field Extractor / Normalizer | 特定レイアウト（era_id）固有の補足構造情報（既知の例外に加え、[ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md)により列位置→意味的フィールド名マッピングを含む） |
...
**`layout` とトップレベル `layouts/` の役割分担**: トップレベルの `layouts/`（[ADR-0003](../adr/0003-layout-definition-strategy.md)）は、様式の**構造定義**（列位置・見出しパターン等）を保持する正データである。`knowledge/` の `layout` カテゴリは、その構造定義だけでは表現しきれない、**特定era_id内で発生する既知の例外・補足知識**（例: 特定月のみ発令日が和暦表記になっている等）に加え、Field Extractorが列位置ベースの汎用フィールド名（`column_1`, `column_2`, ...）で生成した`RawRecord`をNormalizerが意味的に解釈するための**列位置→意味的フィールド名マッピング**（[ADR-0038](../adr/0038-field-extractor-produces-field-extraction-result.md)・[ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md)）を保持する。構造そのものを変えたい場合は `layouts/` を、構造は変えず例外的な扱い・列位置マッピングを追記したい場合は `knowledge/layout` を更新する、という使い分けを行う。物理ディレクトリ名は `layouts/` との混同を避けるため `knowledge/layout_notes/` とする（[カテゴリと物理ディレクトリの対応](#カテゴリと物理ディレクトリの対応)）。列位置マッピングを表現する具体的なエントリ形状（JSON Schema拡張）は[ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md)の時点では未確定であり、Normalizer実装タスクで確定する。
...
```json
{
  ...
  "$defs": {
    ...
    "LayoutEntry": {
      "type": "object",
      "title": "LayoutEntry",
      "description": "特定レイアウト(era_id)に固有の既知の例外・補足知識。knowledge/layout_notes/配下。トップレベルのlayouts/(構造定義)を補足する。列位置→意味的フィールド名マッピング(ADR-0039)を表現する具体的なエントリ形状は本スキーマでは未確定であり、Normalizer実装タスクでの拡張時に別途定義する。",
...
- **列位置→意味的フィールド名マッピング（[ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md)）**: Field Extractorが生成する`RawRecord.raw_fields`は列位置ベースの汎用名（`column_1`, `column_2`, ...、[ADR-0038](../adr/0038-field-extractor-produces-field-extraction-result.md)）で構成される。Normalizerは`RawRecord.layout_id`（era_id）をキーに、本カテゴリのマッピングエントリを`KnowledgeSnapshot`経由で検索し、`column_N`を意味的フィールド名（`name`/`rank`/`organization`等）に変換したうえで正規化する。マッピングエントリの具体的なJSON Schema形状（`LayoutEntry`の拡張、または新エントリ種別の追加）は本ドキュメントでは未確定であり、Normalizer実装タスクで確定する。
...
Validatorは、Normalizer出力に対して `validation` カテゴリの各エントリ（`severity` 別）を適用する。`historical` はNormalizer/Validatorの実行時判定には直接使わず、監査・保守（なぜこの値になっているかの説明）のための知識として位置づける。`layout` は、[ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md)によりNormalizerが列位置→意味的フィールド名マッピングの解決に実行時参照する（上記[適用順序](#normalizervalidatorでの適用順序)の手前、フィールド名解決の前処理として適用する）。それ以外の既知の例外・補足知識としての利用は、引き続き監査・保守のための知識として位置づける。
...
- [ADR-0038](../adr/0038-field-extractor-produces-field-extraction-result.md): Field Extractorが列位置ベースの汎用フィールド名（`column_N`）で`RawRecord`を生成するという前提
- [ADR-0039](../adr/0039-normalizer-field-mapping-via-extended-layout-knowledge.md): `layout` カテゴリのスコープをNormalizerの列位置→意味的フィールド名マッピングにまで拡張する決定
