...
- **責務**: レイアウト判定シグナル（見出しパターン等、`layouts/<era_id>/manifest.yaml`由来）と`Document`を照合し、`LayoutMatch`（`Layout`モデル＋信頼度）を返す。
- **依存先**: `models/`, `utils/`のみ。判定に使う`Layout`データ自体は、呼び出し元（`pipeline/`）から`PipelineContext`経由で渡される（[`pipeline.md`](pipeline.md)）。`layout/`パッケージ自身は`layouts/`ディレクトリのファイルI/Oを行わない。
- **依存禁止**: `extractors/`, `normalizers/`, `validators/`, `repositories/`, `knowledge/`。「Layout Detectorはfieldを知らない」を強制する。

### `sections/`（Section Parser）

- **目的**: 判定されたレイアウト定義に従い、PDF内の対象セクションを切り出す（段階3）。
- **責務**: `Document` + `LayoutMatch` → `list[PersonnelSection]`。
- **依存先**: `models/`, `utils/`のみ。
...
- **責務**: `NormalizedRecord` + `ValidationRuleSet`（呼び出し元から注入） → `ValidationResult`。**レコードの値そのものは変更しない**（[`architecture-contract.md`](../architecture/architecture-contract.md)）。
- **依存先**: `models/`, `utils/`のみ。
- **依存禁止**: `repositories/`, `knowledge/`, `learning/`。検証NGの`learning_dataset`への記録は`validators/`自身ではなく`pipeline/`（呼び出し元）が行う。
