...
- **`JobRunner`**（[`interfaces.md`](interfaces.md#jobrunner)、`pipeline/job_runner.py`、実装済み）は`pipeline/`パッケージの公開窓口であり、`PipelineRunner`の唯一の呼び出し元である。`PipelineContext`を生成し、`knowledge/`から`KnowledgeSnapshot`・`ValidationRuleSet`を取得して各`PipelineStage`（`Normalizer`, `Validator`等）をコンストラクタ注入で構築し、`PipelineBuilder`経由で`PipelineRunner`へ登録した上でその`run()`を呼び出す。`PipelineRunner`が返す`PipelineResult`を`repositories/`（抽象）経由で永続化し、検証NG等の情報を`learning/`へ記録する（[ADR-0013](../adr/0013-learning-dataset-not-correction-log.md)）。`review/`・`export/`には直接依存しない。
- **`JobRunner`のCoordinator責務**（[ADR-0045](../adr/0045-job-runner-aggregate-artifact-coordinator.md)）: Section Parser・Field Extractor・Normalizerは1入力から複数件の集約Artifact（`SectionParseResult.sections`, `FieldExtractionResult.records`, `NormalizationResult.records`）を生成する段階であり、後続段階は単一Artifactを入力として要求する。`PipelineRunner`はこの展開（アンラップ・反復）を行わないため、`JobRunner`が集約Artifactの`tuple`フィールドを読み取り、その要素を入力として`PipelineRunner`を必要な回数だけ構築・呼び出す（文書レベル1回、Section単位・Record単位はそれぞれの件数分）。`JobRunner`は集約Artifactから要素を取り出すのみで、新たな値は生成しない（各段階の出力物生成の独占、[architecture-contract.md 保証10](../architecture/architecture-contract.md)）。

この層構造により、`PipelineContext`生成・Stage列の`run()`の順次呼び出し（1回のPipelineRunner呼び出し内）・`PipelineEvent`記録・`PipelineResult`返却という調整ロジックは`PipelineRunner`が担い、複数回の`PipelineRunner`呼び出しにまたがる反復処理・Repository・Knowledge・Learningとの連携は`JobRunner`が担うという分担になる。個々の`PipelineStage`実装は互いの存在を知らない（[`architecture-contract.md`](../architecture/architecture-contract.md)）。
