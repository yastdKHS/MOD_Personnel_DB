...
| カテゴリ | 主な指標 | 主なデータ源 |
|---|---|---|
| パイプライン実行系 | 実行あたりの経過時間・成功可否・警告件数・エラー件数（`PipelineMetrics`）、Stage別処理時間（`PipelineEvent`の`started`/`completed`イベント間の`timestamp`差分から導出、[ADR-0031](../adr/0031-pipeline-metrics-field-finalization.md)） | `PipelineMetrics` / `PipelineEvent`（[`pipeline.md`](../api/pipeline.md#pipelinemetrics)） |
...
- [ADR-0031](../adr/0031-pipeline-metrics-field-finalization.md) — `PipelineMetrics`のフィールド構成確定。Stage別処理時間を`PipelineEvent`から導出する運用への変更根拠。
