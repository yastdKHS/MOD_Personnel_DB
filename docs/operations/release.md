...
> **実装状況（Phase7 Task16-6時点）**: [`.github/workflows/release.yml`](../../.github/workflows/release.yml)は現在、`workflow_dispatch`・`v*`タグpushをトリガーに、[`ci.yml`](../../.github/workflows/ci.yml)と同じ品質ゲート（ruff lint・ruff format check・mypy・pytest）をPoetry経由で再実行するのみである。下記9段階のうち2（CI検証）・3（マージ）に相当する部分はci.ymlが担い、4はrelease.ymlのトリガー（タグpush）に対応するが、`parser_versions`テーブルへの自動記録自体は未実装である。`ftp/`・`fetch/`パッケージ自体はPhase7（Task16-1・Task16-3）で実装済みであるが、5〜9（`staging`検証・`production`昇格・データ生成・Human Review・公開）に対応する自動化（環境分離・Composition Rootへの配線・CI/CDワークフローからの呼び出し）は未実装であり、現時点では以下のフローは設計目標として記載する。
...
| Composition Root配線・CLI統合 | Phase7（Task16-1〜16-4）で実装済みの`features/`（特徴量計算）・`ftp/`（FTP送信）・`fetch/`（PDF取得）・`services/`（`JobOrchestrator`による横断オーケストレーション）を`cli/`（Composition Root）へ配線し、CLIサブコマンドとして公開する | 同3 |
